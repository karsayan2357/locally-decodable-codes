...
\begin{proof}[Proof of failure probability]
		...
		$\bbd[X] \leq (\delta - \delta^2)(q-1)$. So by applying Chebyshev, we get failure probability in worst case scenario.
		\begin{align*}
				\Pr\left[X\geq \frac{1}{2}(1-2\sigma)(q-1)\right]
				& \leq \frac{\bbd[X]}{((1-2\sigma)(q-1)/2 - (q-1)\delta)^2} \\[0.2em]
				& = O_{\delta, \sigma}(1/q).
		\end{align*}
\end{proof}
...
\begin{proof}[Proof of failure probability]
	...
	Now similary we get that $\bbe[X]\leq \delta(q-1)B$ and $\bbd[X]\leq (\delta - \delta^2)(q-1)B^2$, hence applying Chebyshev, we get out desired bound for the failure probability.
\end{proof}
